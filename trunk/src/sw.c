 ...
 * sender sequence variable
 */
short last_packet_sent=1000;  //send next packet
short last_packet_acked=1000; //last byte ack
short send_window=4;  //advertised  window, max rcv buffer, last bye written

short send_congestion_window=6;


short initial_seq_number=1000;


/*
 * receiver sequence  variable
 */
short rcvWindow=4; //receive window
short last_byte_received=1003; //receive next packet , last byte written
short next_byte_expected=1000;
short init_rec_seq_number=0;
...
int sender_send_packet( short send_seq_number){
	 printf("error1111! lastsen: %d, lastAck: %d , seq se:  %d", last_packet_sent, last_packet_acked,send_seq_number);

    if (last_packet_sent-last_packet_acked < min(send_window,send_congestion_window))
        {
			last_packet_sent+=1;
...
enum ack sender_receive_ack( short send_seq_number){
	printf("\n eeeeeeerror 0  senSq:  %d, lastAck: %d , last sen:  %d!", send_seq_number, last_packet_acked, last_packet_sent);

	if ((send_seq_number == last_packet_acked +1) && (last_packet_acked<last_packet_sent) && (last_packet_sent<=(send_window+last_packet_acked))){
						last_packet_acked=last_packet_acked+1;
						printf("error 3!");

						return correct_ack;
					}
	 else if (last_packet_acked<send_seq_number && send_seq_number<(send_window+last_packet_acked)){
	 		printf("error2!");

		 return in_window_ack;
	 		}

	 else if (send_seq_number<last_packet_acked && send_seq_number > (send_window+last_packet_acked)){
			printf("error1!");
